...
Action = namedtuple("action", ["rotation"])
...
class ActionParam:
    ...
    def get(self):
        self.rotation = torch.from_numpy(
            np.array(self.rotation).reshape(len(self.rotation), 1)
        )
        return self
    ...
    def get2(self):
        return Variable(
            torch.from_numpy(
                np.concatenate([np.array(self.rotation).reshape(len(self.rotation), 1)])
            )
        ).float()
...
class Actor(nn.Module):
    ...
    def forward(self, inp):
        ...
        # print()
        # exit()
        x[:, 0] = self.max_action[0] * F.tanh(x[:, 0])
        # x[:, 1] = self.max_action[1] * F.sigmoid(x[:, 1])
        # print(x.shape)

        action = ActionParam()
        for i in x:
            action.add(Action(i.float().detach()))
        return action.get()
...
class TD3(object):
    ...
    def select_action(self, state):
        ...
        action = self.actor(state_wrapper)
        action.rotation = action.rotation.cpu().data.numpy().flatten()
        return action
    ...
    def train(
        ...
        for it in range(iterations):
            ...
            noise = (
                torch.Tensor(batch_actions.rotation)
                .data.normal_(0, policy_noise)
                .to(device)
            )
            noise = noise.clamp(-noise_clip, noise_clip)
            noise = noise.reshape(batch_size, len(self.max_action))

            next_action.rotation = (next_action.rotation + noise).clamp(
                -self.max_action[0], self.max_action[0]
            )
